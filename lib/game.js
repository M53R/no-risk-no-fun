...
function createRoom(defaultHearts = 3) {
  const room = {
    ...
    currentTurn: null,
    groups: [],
    showRedsToPlayers: true,   // خيار المقدم: هل تظهر الحمراء المعروضة بشاشات اللاعبين؟
...
function calculateWinner(room) {
  ...
  // إعلان الفائز = كشف كل شيء (الزرقاء تظهر على شاشة المقدم الآن فقط)
  room.reveals.blueAll = true;
  room.reveals.redAll = true;
  room.phase = 'results';
  room.roundActive = false;
...
function playerCanSee(room, viewerId, owner, kind) {
  ...
  if (kind === 'blue') {
    ...
    return r.blueAll || !!per.blue;
  }
  if (viewerId === owner.id) return true;
  if (r.redAll || per[kind]) return true;
  return room.showRedsToPlayers && owner.cards.shownRed === kind;
}

function hostCanSee(room, owner, kind) {
  const r = room.reveals;
  const per = r.perPlayer[owner.id] || {};
  if (kind === 'blue') return r.blueAll || !!per.blue;          // شاشة المقدم للعرض: الزرقاء مخفية حتى إعلان الفائز
  if (r.redAll || per[kind]) return true;
  return owner.cards.shownRed === kind;                          // المعروضة فقط
}
...
module.exports = {
  ...
  setTurn, pickCard, setShownRed, startRedPhase, setRoundAction, setShowReds,
