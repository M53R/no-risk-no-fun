...
function bootFromDb() {
  for (const room of loadAllRooms()) {
    ...
    if (room.hostName === undefined) room.hostName = '';
...
function createRoom(defaultHearts = 3) {
  const room = {
    ...
    hostName: '',              // يظهر تحت الشعار في الانترو (إن كُتب)
...
// اسم المقدم — يظهر تحت الشعار في الانترو
function setHostName(room, name) {
  const prev = room.hostName;
  room.hostName = String(name || '').trim().slice(0, 30);
  addLog(room.code, 'تغيير اسم المقدم', null, prev || '—', room.hostName || '—', null);
  saveRoom(room);
  return {};
}
...
module.exports = {
  ...
  setTurn, pickCard, setShownRed, startRedPhase, setRoundAction, cancelRoundAction, setShowReds, setHostName,
